...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.heads = [self.SingleHeadAttention(embedding_dim, head_size) for _ in range(num_heads)]
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        for head in self.heads:
            a = head(embedded)
            assert a.shape[:2] == (batch_size, length) #, attention_dim
            z.append(a)
        
        z = torch.cat(z, axis=2)
        # z = self.linear(z)

        return z.round(decimals=4)
